...
CnHeader::CnHeader(const uint16_t fid,
                   uint8_t qIndex,
                   uint8_t ecnbits,
                   uint16_t qfb,
                   uint16_t total)
    : m_fid(fid),
      ...
      m_ecnBits(ecnbits)
{
    CnHeader_t.m_qfb = qfb;
    CnHeader_t.m_total = total;
...
void
CnHeader::SetFlow(const uint16_t fid)
{
    m_fid = fid;
}

void
CnHeader::SetQindex(const uint8_t qIndex)
{
    m_qIndex = qIndex;
}

void
CnHeader::SetQfb(uint16_t q)
{
    CnHeader_t.m_qfb = q;
}

void
CnHeader::SetTotal(uint16_t total)
{
    CnHeader_t.m_total = total;
}

void
CnHeader::SetECNBits(const uint8_t ecnbits)
{
    m_ecnBits = ecnbits;
}

uint16_t
CnHeader::GetFlow() const
{
    return m_fid;
}

uint8_t
CnHeader::GetQindex() const
{
    return m_qIndex;
}

uint16_t
CnHeader::GetQfb() const
{
    return CnHeader_t.m_qfb;
}

uint16_t
CnHeader::GetTotal() const
{
    return CnHeader_t.m_total;
}

uint8_t
CnHeader::GetECNBits() const
{
    return m_ecnBits;
}

void
CnHeader::SetSeq(const uint32_t seq)
{
    m_seq = seq;
}

uint32_t
CnHeader::GetSeq() const
{
    return m_seq;
}
...
{
    static TypeId tid = TypeId("ns3::CnHeader").SetParent<Header>().AddConstructor<CnHeader>();
    return tid;
}
...
void
CnHeader::Print(std::ostream& os) const
{
    // m_fid.Print(os);
    os << " qFb=" << (unsigned)CnHeader_t.m_qfb << "/" << (unsigned)CnHeader_t.m_total;
}

uint32_t
CnHeader::GetSerializedSize(void) const
{
    return 8;
}

void
CnHeader::Serialize(Buffer::Iterator start) const
{
    ...
    start.WriteU16(CnHeader_t.m_qfb);
    start.WriteU16(CnHeader_t.m_total);
    // NS_LOG_LOGIC("CN Seriealized as " << std::hex << hibyte << "+" << lobyte << std::dec);
}

uint32_t
CnHeader::Deserialize(Buffer::Iterator start)
{
    ...
    CnHeader_t.m_qfb = start.ReadU16();
    CnHeader_t.m_total = start.ReadU16();
