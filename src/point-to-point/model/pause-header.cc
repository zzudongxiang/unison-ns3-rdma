...
void
PauseHeader::SetTime(uint32_t time)
{
    m_time = time;
}

uint32_t
PauseHeader::GetTime() const
{
    return m_time;
}

void
PauseHeader::SetQLen(uint32_t qlen)
{
    m_qlen = qlen;
}

uint32_t
PauseHeader::GetQLen() const
{
    return m_qlen;
}

void
PauseHeader::SetQIndex(uint8_t qindex)
{
    m_qindex = qindex;
}

uint8_t
PauseHeader::GetQIndex() const
{
    return m_qindex;
}
...
{
    static TypeId tid =
        TypeId("ns3::PauseHeader").SetParent<Header>().AddConstructor<PauseHeader>();
    return tid;
}
...
void
PauseHeader::Print(std::ostream& os) const
{
    os << "pause=" << m_time << "us, qlen=" << m_qlen << ", qidx=" << int(m_qindex);
}

uint32_t
PauseHeader::GetSerializedSize(void) const
{
    return 9;
}

void
PauseHeader::Serialize(Buffer::Iterator start) const
{
...
uint32_t
PauseHeader::Deserialize(Buffer::Iterator start)
{
