...
TypeId
SimpleDropTailQueue::GetTypeId(void)
{
    static TypeId tid =
        TypeId("ns3::SimpleDropTailQueue")
            .SetParent<PacketQueue>()
            .AddConstructor<SimpleDropTailQueue>()
            .AddAttribute(
                "Mode",
                "Whether to use bytes (see MaxBytes) or packets (see MaxPackets) as the maximum "
                "queue size metric.",
                EnumValue(QUEUE_MODE_BYTES),
                MakeEnumAccessor<SimpleDropTailQueue::QueueMode>(&SimpleDropTailQueue::SetMode),
                MakeEnumChecker(QUEUE_MODE_BYTES,
                                "QUEUE_MODE_BYTES",
                                QUEUE_MODE_PACKETS,
                                "QUEUE_MODE_PACKETS"))
